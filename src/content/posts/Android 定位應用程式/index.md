...
我們不直接使用 `PackageManager.PERMISSION_GRANTED` 進行判斷，是因為 **`PackageManager.PERMISSION_GRANTED` 只是一個「結果程式碼」（密碼）**，而 **`ContextCompat.checkSelfPermission()` 才是「執行檢查的動作」**。
